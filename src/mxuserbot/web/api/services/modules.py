...
class ModuleService:
    ...
    async def preview_module(
        ...
        url, _ = await repo.resolve_and_download(target)
        if not url:
            ...
            with zipfile.ZipFile(io.BytesIO(raw)) as zf:
                for name in zf.namelist():
                    ...
                    if info.file_size > 512 * 1024:
                        files.append({"name": name, "content": None, "size": info.file_size, "binary": True})
                        continue
                    raw_bytes = zf.read(name)
                    try:
                        text = raw_bytes.decode("utf-8")
                        files.append({"name": name, "content": text, "size": info.file_size, "binary": False})
                    except UnicodeDecodeError:
                        files.append({"name": name, "content": None, "size": info.file_size, "binary": True})
            return {"type": "zip", "files": files}

        code = await utils.request(url, return_type="text")
        filename = url.split("/")[-1]
        return {
            "type": "py",
            "files": [{"name": filename, "content": code, "size": len(code), "binary": False}],
        }

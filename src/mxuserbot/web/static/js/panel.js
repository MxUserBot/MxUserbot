...
createApp({
    setup() {
        ...
        const previewModal = ref({ open: false, loading: false, target: '', mod: null, type: '', files: [], selectedFile: null, confirmLoading: false });
        ...
        const openPreview = async (mod) => {
            const target = mod.target || mod.raw_url;
            if (!target) return;
            previewModal.value = { open: true, loading: true, target, mod, type: '', files: [], selectedFile: null, confirmLoading: false };
            try {
                const res = await fetch(`/api/modules/preview?target=${encodeURIComponent(target)}`);
                if (!res.ok) { closePreview(); return; }
                const data = await res.json();
                previewModal.value.type = data.type;
                previewModal.value.files = data.files;
                if (data.files.length > 0) previewModal.value.selectedFile = data.files[0];
            } catch (e) { closePreview(); }
            previewModal.value.loading = false;
        };

        const previewSelectFile = (file) => { previewModal.value.selectedFile = file; };

        const closePreview = () => { previewModal.value = { open: false, loading: false, target: '', mod: null, type: '', files: [], selectedFile: null, confirmLoading: false }; };

        const confirmInstall = async () => {
            const mod = previewModal.value.mod;
            if (!mod) return;
            previewModal.value.confirmLoading = true;
            closePreview();
            await toggleModule(mod);
        };
        ...
        return { view, modTab, loading, processing, stats, searchQuery, currentModList, btnClass, tabClass, searchModules, toggleModule, cfgModal, openConfig, closeConfig, saveConfig, errors, repos, newRepo, repoLoading, addRepo, removeRepo, updateHost, currentHostMode, hostSaving, handleLogout, previewModal, openPreview, previewSelectFile, closePreview, confirmInstall };
    }
}).mount('#app');